...
CONFIG = {
    ...
    'ACCURACY_WINDOW': 100,  # Number of predictions to track for accuracy
    'ACCURACY_THRESHOLD': 0.5,  # Retrain if accuracy falls to or below 50%
    'MIN_OBSERVATIONS_FOR_ACCURACY': 50  # Minimum predictions before checking accuracy
}
...
def calculate_indicators(df: pd.DataFrame, sentiment: float) -> pd.DataFrame:
    """Calculate technical indicators and future trend."""
    df = df.copy()
    ...
    df['Future_Trend'] = np.where(df['close'].shift(-1) > df['close'], 1, 0)  # Predict next bar's price movement
    
    indicator_cols = [
        'MA20', 'MA50', 'RSI', 'MACD', 'MACD_signal', 'OBV', 'VWAP', 'ATR',
        'CMF', 'Close_ATR', 'MA20_ATR', 'Return_1d', 'Return_5d', 'Volatility',
        'BB_upper', 'BB_middle', 'BB_lower', 'Stoch_K', 'Stoch_D', 'ADX', 'Sentiment', 'Future_Trend'
    ]
    df = df.dropna(subset=indicator_cols[:-1])  # Exclude Future_Trend from dropna for live trading
    return df
...
def validate_indicators(df: pd.DataFrame, symbol: str) -> None:
    ...
    required_cols = [
        ...
        'MA20', 'MA50', 'RSI', 'MACD', 'MACD_signal', 'OBV', 'VWAP', 'ATR',
        'CMF', 'Close_ATR', 'MA20_ATR', 'Return_1d', 'Return_5d', 'Volatility',
        'BB_upper', 'BB_lower', 'Stoch_K', 'Stoch_D', 'ADX', 'Sentiment', 'Future_Trend'
    ]
    if not all(col in df.columns for col in required_cols):
        missing = [col for col in required_cols if col not in df.columns]
...
def preprocess_data(df: pd.DataFrame, timesteps: int, add_noise: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    """Preprocess data into sequences for training or prediction."""
    df = df.copy()
    features = [
        'close', 'high', 'low', 'volume', 'VWAP', 'MA20', 'MA50', 'RSI',
        'MACD', 'MACD_signal', 'OBV', 'ATR', 'CMF', 'Close_ATR',
        'MA20_ATR', 'Return_1d', 'Return_5d', 'Volatility', 'BB_upper',
        'BB_lower', 'Stoch_K', 'Stoch_D', 'ADX', 'Sentiment'
    ]
    if add_noise:
        df = df.dropna(subset=['Future_Trend'])  # For training, ensure Future_Trend is available
        X = df[features].values
        y = df['Future_Trend'].values
    else:
        X = df[features].values
        y = None  # For live trading, y is not needed
    
    # Handle NaNs
    X = pd.DataFrame(X, columns=features).ffill().fillna(0).values
    ...
    if add_noise:
        ...
        for i in range(len(X) - timesteps):
            ...
            y_seq.append(y[i + timesteps - 1])  # Predict Future_Trend of the last bar in sequence
...
class OverfittingStop(Callback):
    """Custom callback to stop training if validation loss significantly exceeds training loss."""
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if train_loss is None or val_loss is None:
            return
        if val_loss > self.threshold * train_loss:
            self.overfit_count += 1
            logger.warning(f"Epoch {epoch + disfr1}: Potential overfitting detected (val_loss: {val_loss:.4f}, train_loss: {train_loss:.4f})")
            if self.overfit_count >= self.patience:
                self.model.stop_training = True
                logger.info(f"Training stopped for {self.model.name or 'model'} due to overfitting after {self.patience} epochs of divergence")
        else:
            self.overfit_count = 0
...
def train_model(symbol: str, X: np.ndarray, y: np.ndarray, epochs: int, batch_size: int, progress_bar) -> Tuple[tf.keras.Model, StandardScaler]:
    """Train the model with early stopping and overfitting prevention."""
    ...
    overfitting_stop = OverfittingStop(threshold=1.5, patience=3)
    ...
    model.fit(
        ...
        callbacks=[early_stopping, TQDMCallback(progress_bar), overfitting_stop],
        verbose=0
    )
...
def backtest(
...
) -> Tuple[float, List[float], int, float, List[float]]:
    """Run a backtest simulation with enhanced features."""
    X, _ = preprocess_data(df, timesteps, add_noise=False)
    if X.size == 0:
        return initial_cash, [], 0, 0.0, [initial_cash]
    ...
    for i in range(k_start, len(predictions)):
        ...
        if atr_val <= 0:
            logger.warning(f"Invalid ATR value ({atr_val}) for {symbol} at {ts}, skipping trade")
            equity_curve.append(cash + (position * price if position > 0 else 0))
            continue
        ...
        if pred > buy_threshold and position == 0 and qty > 0 and current_rsi < 30 and current_adx > adx_trend_threshold:
            position = qty
            entry_price = price * (1 + slippage_percentage)
            max_price = entry_price
            entry_time = ts
            trade_cost = qty * entry_price + transaction_cost_per_trade
            if trade_cost <= cash:
                cash -= trade_cost
                logger.info(f"{ts}: Bought {qty} shares of {symbol} at ${entry_price:.2f}, cash: ${cash:.2f}")
            else:
                logger.warning(f"Insufficient cash for buy: {trade_cost} > {cash}")
                position = 0
                qty = 0
            ...
            if time_held >= min_holding_period_minutes:
                if price <= trailing_stop or price <= stop_loss or price >= take_profit or (pred < sell_threshold and current_rsi > 70):
                    sell_price = price * (1 - slippage_percentage)
                    cash += position * sell_price - transaction_cost_per_trade
                    ret = (sell_price - entry_price) / entry_price
                    returns.append(ret)
                    trade_count += 1
                    if ret > 0:
                        winning_trades += 1
                    logger.info(f"{ts}: Sold {position} shares of {symbol} at ${sell_price:.2f}, return: {ret:.3f}, cash: ${cash:.2f}")
                    position = 0
                    entry_time = None
                    max_price = 0.0
            equity_curve.append(cash + (position * price))
    ...
    if position > 0:
        sell_price = prices[-1] * (1 - slippage_percentage)
        cash += position * sell_price - transaction_cost_per_trade
        ret = (sell_price - entry_price) / entry_price
        returns.append(ret)
        trade_count += 1
        if ret > 0:
            winning_trades += 1
        logger.info(f"Final sell: Sold {position} shares of {symbol} at ${sell_price:.2f}, return: {ret:.3f}, cash: ${cash:.2f}")
        equity_curve.append(cash)

    cash = max(cash, 0)
    win_rate = (winning_trades / trade_count * 100) if trade_count > 0 else 0.0
    return cash, returns, trade_count, win_rate, equity_curve

def calculate_performance_metrics(returns: List[float], cash: float, initial_cash: float, equity_curve: List[float]) -> Dict[str, float]:
    """Calculate performance metrics with corrected Sharpe ratio and drawdown."""
...
def main(backtest_only: bool = False, force_train: bool = False) -> None:
    """Main function with accuracy monitoring and portfolio-level risk management."""
    ...
    expected_features = 24  # Adjusted for features list
    models = {}
    scalers = {}
    dfs = {}
    stock_info = []
    ...
    if not backtest_only:
        ...
        accuracy_tracker = {symbol: deque(maxlen=CONFIG['ACCURACY_WINDOW']) for symbol in CONFIG['SYMBOLS']}
        previous_predictions = {symbol: None for symbol in CONFIG['SYMBOLS']}
                        ...
                        if X.size > 0:
                            ...
                            if current_volatility <= CONFIG['MAX_VOLATILITY'] and current_adx >= 20 and prediction >= CONFIG['CONFIDENCE_THRESHOLD']:
                                if prediction > CONFIG['PREDICTION_THRESHOLD_BUY'] and qty_owned == 0 and current_rsi < 30 and current_adx > CONFIG['ADX_TREND_THRESHOLD']:
                                    ...
                                    trade_cost = qty * price + CONFIG['TRANSACTION_COST_PER_TRADE']
                                    if qty > 0 and trade_cost <= cash:
...
"""
                            ...
                            # Accuracy monitoring
                            if previous_predictions[symbol] is not None:
                                prev_pred, prev_ts = previous_predictions[symbol]
                                if prev_ts < df['timestamp'].iloc[-1]:
                                    actual_trend = df['Future_Trend'].iloc[-2]  # Last closed bar's Future_Trend
                                    predicted_trend = 1 if prev_pred > 0.5 else 0
                                    accuracy_tracker[symbol].append(predicted_trend == actual_trend)
                                    if len(accuracy_tracker[symbol]) >= CONFIG['MIN_OBSERVATIONS_FOR_ACCURACY']:
                                        accuracy = np.mean(accuracy_tracker[symbol])
                                        logger.info(f"Accuracy for {symbol}: {accuracy:.2f}")
                                        if accuracy <= CONFIG['ACCURACY_THRESHOLD']:
                                            logger.info(f"Retraining model for {symbol} due to low accuracy: {accuracy:.2f}")
                                            df_full, _ = load_or_fetch_data(symbol, CONFIG['TRAIN_DATA_START_DATE'], datetime.now().strftime('%Y-%m-%d'))
                                            df_full = calculate_indicators(df_full, sentiment)
                                            X, y = preprocess_data(df_full, CONFIG['TIMESTEPS'], add_noise=True)
                                            model, scaler = train_model(symbol, X, y, CONFIG['TRAIN_EPOCHS'], CONFIG['BATCH_SIZE'], progress_bar)
                                            models[symbol] = model
                                            scalers[symbol] = scaler
                                            accuracy_tracker[symbol].clear()
                                            send_email(f"Model Retrained: {symbol}", f"Model for {symbol} retrained due to accuracy {accuracy:.2f} <= {CONFIG['ACCURACY_THRESHOLD']}")

                            previous_predictions[symbol] = (prediction, df['timestamp'].iloc[-1])
        ...
        for symbol in CONFIG['SYMBOLS']:
            if symbol in models:
                cash, returns, trade_count, win_rate, equity_curve = backtest(
                ...
                symbol_results[symbol] = calculate_performance_metrics(returns, cash, initial_cash / len(CONFIG['SYMBOLS']), equity_curve)
