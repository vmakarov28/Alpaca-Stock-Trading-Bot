...
# | - Ensure Alpaca API keys and details configured in CONFIG.                   |
...
# |                                                                              |
# +------------------------------------------------------------------------------+
#Add stop feture to prevent overfitting during training when model stops improving
...
CONFIG = {
    ...
    'ALPACA_API_KEY': 'REDACTED', # Alpaca API key for accessing market data and trading
    'ALPACA_SECRET_KEY': 'REDACTED', # Alpaca secret key for authentication
    'EMAIL_SENDER': 'REDACTED@gmail.com', # Email address for sending trade notifications
    'EMAIL_PASSWORD': 'REDACTED', # Password for the sender email account
    'EMAIL_RECEIVER': ['REDACTED@gmail.com', 'REDACTED@hotmail.com'], # List of email addresses to receive notifications
    ...
    'PREDICTION_THRESHOLD_BUY': 0.6, # Minimum model prediction confidence for a buy decision
    'PREDICTION_THRESHOLD_SELL': 0.4, # Maximum model prediction confidence for a sell decision
...
#-------- ⬇ CONFIGUE FOLLOWING VARIABLES TO CHANGE TRADING SYLES ⬇ -----------------------------
    'CONFIDENCE_THRESHOLD': 0.55, # Minimum model prediction confidence required to consider a trading decision
    'MAX_VOLATILITY': 3, # Maximum allowable price volatility to permit trading actions
    'ADX_TREND_THRESHOLD': 30, # Minimum ADX value indicating a strong trend to allow buy decisions
    'RISK_PERCENTAGE': 1, # Percentage of available cash to risk per trade for position sizing
    'STOP_LOSS_ATR_MULTIPLIER': 1.1, # Multiplier of ATR to set the stop-loss distance below entry price
    'TAKE_PROFIT_ATR_MULTIPLIER': 1.5, # Multiplier of ATR to set the take-profit target above entry price
                        ...
                        if X.shape[0] > 0:
                            ...
                            if current_volatility <= CONFIG['MAX_VOLATILITY'] and current_adx >= CONFIG['ADX_TREND_THRESHOLD'] and prediction >= CONFIG['CONFIDENCE_THRESHOLD']:
                                if prediction > CONFIG['PREDICTION_THRESHOLD_BUY'] and qty_owned == 0 and current_rsi < 30 and current_adx > CONFIG['ADX_TREND_THRESHOLD']:
                                    decision = "Buy"
                            #Makes buy when hold wiith no positions
