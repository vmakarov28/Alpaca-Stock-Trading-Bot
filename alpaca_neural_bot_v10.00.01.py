...
from alpaca.data import StockHistoricalDataClient, TimeFrame, TimeFrameUnit, NewsClient  # For Alpaca historical data + NEWS
from alpaca.data.requests import StockBarsRequest, NewsRequest  # For requesting stock bar data and NEWS from Alpaca API
...
from hmmlearn.hmm import GaussianHMM  # NEW: Hidden Markov Model for regime detection
import xgboost as xgb  # NEW for ensemble
import triton
...
CONFIG = {
    ...
    'ALPACA_API_KEY': 'key',  # API key for Alpaca
    'ALPACA_SECRET_KEY': 'key',  # Secret key for Alpaca
    ...
    'EMAIL_PASSWORD': 'a',  # Password for the email account
    'EMAIL_RECEIVER': ['@gmail.com', '@students.d125.org', '@hotmail.com'],  # List of email recipients
    ...
    'MODEL_VERSION': 'v31026',  # Model architecture version; increment on structural changes to force retrain
...
#python /mnt/c/Users/aipla/Downloads/deepTrader10.00.01.py --backtest --force-train --DEBUG
...
def get_sentiment_score(symbol: str) -> float:
    """
    REAL sentiment using Alpaca News API + transformers pipeline.
    Handles dict, object, and tuple responses from Alpaca (robust).
    Cached for 1 hour. Returns score between -1.0 and +1.0.
    """
...
def is_cointegrated(series1: pd.Series, series2: pd.Series, pvalue_threshold: float = 0.05) -> bool:
    """Test if two price series are cointegrated (Engle-Granger) with safe alignment."""
...
def calculate_spread(df1: pd.DataFrame, df2: pd.DataFrame) -> pd.Series:
    """Compute cointegrated spread with safe alignment — no FutureWarning."""
...
def fetch_data(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
    """Fetch historical bar data from Alpaca API in yearly increments with extra retry + cache fallback."""
...
def load_or_fetch_data(symbol: str, start_date: str, end_date: str) -> Tuple[pd.DataFrame, bool]:
    """Load historical data from cache or fetch from API. Uses date-range specific cache to prevent backtest contamination."""
...
def calculate_indicators(df: pd.DataFrame, sentiment: float) -> pd.DataFrame:
    """Calculate technical indicators with better features: multi-timeframe, volume profile, macro proxy via volatility, earnings proxy via sentiment boost."""
    df = df.copy()
...
def preprocess_data(df: pd.DataFrame, timesteps: int, add_noise: bool = False, inference_scaler: Optional[RobustScaler] = None, inference_mode: bool = False, fit_scaler: bool = True) -> Tuple[np.ndarray, Optional[np.ndarray], Optional[RobustScaler]]:
    """Preprocess data into sequences for future price direction prediction with better features (32 features now)."""
    df = df.copy()
...
class TradingModel(nn.Module):
    """S+++ Optimized: Real LSTM + MultiheadAttention + checkpointing (torch.compile disabled due to known inductor bug — safe eager fallback)"""
...
def train_model(symbol: str, worker_id: int, df: pd.DataFrame, epochs: int, batch_size: int, timesteps: int, expected_features: int, barrier=None, gpu_semaphore=None, preprocessed_train=None, preprocessed_val=None) -> Tuple[nn.Module, Any, GaussianHMM, Any]:
    """S+++ FULL TRAINING: compile + streams + prefetch + checkpoint (no quality loss)"""
...
def train_hmm(X_scaled: np.ndarray, num_regimes: int = CONFIG['NUM_REGIMES']) -> GaussianHMM:
    """Train Hidden Markov Model with safe diagonal covariance + fallback."""
    try:
        hmm = GaussianHMM(
...
def train_xgboost(X_train, y_train, X_val, y_val):
    """Train XGBoost classifier for ensemble."""
...
def load_model_and_scaler(symbol: str, expected_features: int, force_retrain: bool = False) -> Tuple[Optional[nn.Module], Optional[RobustScaler], Optional[float], Optional[GaussianHMM]]:
    """Load trained model, scaler, sentiment, and HMM from cache or return None to trigger training."""
...
def save_model_and_scaler(symbol: str, model: nn.Module, scaler: RobustScaler, sentiment: float, hmm: GaussianHMM) -> None:
    """Save the trained model, scaler, sentiment, and HMM to cache files."""
    try:
        model_path = os.path.join(CONFIG['MODEL_CACHE_DIR'], f"{symbol}_model_{CONFIG['MODEL_VERSION']}.pth")
        scaler_path = os.path.join(CONFIG['MODEL_CACHE_DIR'], f"{symbol}_scaler_{CONFIG['MODEL_VERSION']}.pkl")
        sentiment_path = os.path.join(CONFIG['MODEL_CACHE_DIR'], f"{symbol}_news_sentiment_{CONFIG['MODEL_VERSION']}.pkl")
...
def train_symbol(symbol: str, worker_id: int, expected_features: int, force_train: bool, barrier=None, gpu_semaphore=None, preprocessed_train=None, preprocessed_val=None, backtest_only: bool = False, debug: bool = False) -> Tuple[str, nn.Module, Any, bool, float, bool, bool, GaussianHMM, Any]:
    """Train or load model for a symbol - now returns XGBoost too. barrier is used for GPU sync."""
...
def backtest(symbol: str, model: nn.Module, scaler: RobustScaler, df: pd.DataFrame, initial_cash: float,
    ...
    """Optimized backtest with toggle for full vs fast pairs resolution."""
...
def buy_and_hold_backtest(dfs_backtest: Dict[str, pd.DataFrame], initial_cash: float) -> Tuple[float, Dict[str, pd.Series]]:
    """Fixed: Buy-and-Hold now starts on the exact same date as the neural backtest (Jan 1, 2025)."""
    backtest_start = pd.Timestamp(CONFIG['BACKTEST_START_DATE'], tz='UTC')
...
def make_prediction(model: nn.Module, X: np.ndarray, xgb_model=None) -> float:
    """Safe prediction with temperature scaling for sharper decisions."""
...
"""
                            ...
                            if price <= trailing_stop or price <= stop_loss or price >= take_profit or \
                               ...
                               (prediction < CONFIG['CONFIDENCE_THRESHOLD'] - 0.05):
                                decision = "Sell"
                                # your sell order code stays exactly here
