...
#CHOOSE 1 COMMAND TO RUN IN CMD
#python alpaca_neural_bot_v2_4.py #Live trading, use cached models
#python alpaca_neural_bot_v2_4.py --force-train #Live trading, retrain models
#python alpaca_neural_bot_v2_4.py --backtest #Backtest, use cached models
#python alpaca_neural_bot_v2_4.py --backtest --force-train #Backtest, retrain models
...
#Customize this for different trading styles
    'CONFIDENCE_THRESHOLD': 0.55,  # New: Confidence threshold for trades 0<x<1
    'MAX_VOLATILITY': 2,  # New: No-trade zone volatility limit 0<x<5
    'ADX_TREND_THRESHOLD': 30,  # Trend Strength, 20<x40 but technically 0<x<100
    'RISK_PERCENTAGE': 1, # New: Risk percentage for position sizing. 0<x<=1
...
def load_news_sentiment(symbol: str) -> Tuple[float, bool]:
    ...
    if os.path.exists(cache_file) and (time.time() - os.path.getmtime(cache_file)) < CONFIG['CACHE_EXPIRY_SECONDS']:
        ...
        # Placeholder: Replace with actual news fetching logic
...
def backtest(
    ...
    adx = df['ADX'].iloc[timesteps:].values  # New: ADX for trend detection
    volatility = df['Volatility'].iloc[timesteps:].values  # New: Volatility for no-trade zone
    ...
    for i in range(k_start, len(predictions)):
        ...
        ts = pd.Timestamp(sim_timestamps[i])  # Ensure ts is a pandas Timestamp

        # Skip trade if volatility is too high or ADX indicates no trend
        ...
        # Buy logic: prediction > threshold, RSI < 30, and ADX > 25
        if pred > buy_threshold and position == 0 and qty > 0 and current_rsi < 30 and current_adx > adx_trend_threshold:
            position = qty
            entry_price = price
            max_price = price  # Initialize max_price for trailing stop
            entry_time = ts
            ...
            time_held = (ts - entry_time).total_seconds() / 60  # Calculate time held in minutes

            if time_held >= min_holding_period_minutes:
                # Sell if trailing stop, stop-loss, take-profit, or sell conditions are met
                ...
                for symbol in CONFIG['SYMBOLS']:
                    if symbol in models:
                        ...
                        if X.shape[0] > 0:
                            ...
                            # Skip trade if volatility too high or ADX too low
                            if current_volatility > CONFIG['MAX_VOLATILITY'] or current_adx < 20:
                                continue
