# alpaca_neural_bot_v1_9.py
# Version 1.9 - Enhanced to reduce overfitting with stronger regularization and data augmentation,
#               improved data fetching with customizable start dates and pagination, fixed live trading errors,
#               fixed data loading and training progress issues, and fixed datetime comparison errors.
#
# Possible Run Commands:
# 1. Backtest mode (using cached models):
#    python alpaca_neural_bot_v1_9.py --backtest
# 2. Backtest mode (forcing retraining):
#    python alpaca_neural_bot_v1_9.py --backtest --force-train
# 3. Live trading mode (using cached models):
#    python alpaca_neural_bot_v1_9.py
# 4. Live trading mode (forcing retraining):
#    python alpaca_neural_bot_v1_9.py --force-train
...
def fetch_data(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
    """Fetch historical bar data from Alpaca API with improved pagination support."""
    try:
        client = StockHistoricalDataClient(CONFIG['ALPACA_API_KEY'], CONFIG['ALPACA_SECRET_KEY'])
        all_bars = []
        page_limit = 10000  # Alpaca's maximum page size
        # Convert start_date and end_date to offset-aware UTC timestamps
        ...
        while current_end > start_dt:
            ...
            # Update the end date to the earliest timestamp fetched minus 1 second
            current_end = df_bars['timestamp'].min() - pd.Timedelta(seconds=1)
            # Avoid infinite loops if no more data is available
            if len(bars) < page_limit and current_end > start_dt:
                # Move the end date back to avoid overlapping requests
