# alpaca_neural_bot_v2_2.py
# Version 2.2 - Enhanced with dynamic stop-losses, real-time sentiment analysis, portfolio-level risk management,
#               reduced overtrading, and improved model generalization. All previous features are maintained.
...
from transformers import pipeline  # For real-time sentiment analysis
...
CONFIG = {
    'SYMBOLS': ['NVDA', 'TSLA', 'AAPL', 'MSFT', 'AMZN'],  # Expanded for diversification
    ...
    'STOP_LOSS_ATR_MULTIPLIER': 1.5,  # Renamed for clarity, now ATR-based
    'TAKE_PROFIT_ATR_MULTIPLIER': 2.0,  # Renamed for clarity, now ATR-based
    'TIMESTEPS': 30,
    'MIN_DATA_POINTS': 100,
    'CACHE_DIR': './cache',
    ...
    'LIVE_DATA_BARS': 200,
    'MIN_HOLDING_PERIOD_MINUTES': 60,  # New: Minimum holding period to reduce overtrading
    'MAX_DRAWDOWN_LIMIT': 0.20,  # New: Portfolio-level drawdown limit (20%)
    'TRANSACTION_COST_PER_TRADE': 1.0,  # New: Fixed transaction cost per trade in dollars
    'SENTIMENT_MODEL': 'distilbert-base-uncased-finetuned-sst-2-english'  # New: Pre-trained sentiment model
}
...
# Initialize sentiment analysis pipeline (New)
...
def validate_config(config: Dict) -> None:
    ...
    for param in ['INITIAL_CASH', 'STOP_LOSS_ATR_MULTIPLIER', 'TAKE_PROFIT_ATR_MULTIPLIER', 'MAX_DRAWDOWN_LIMIT']:
...
def load_news_sentiment(symbol: str) -> Tuple[float, bool]:
    """Compute real-time news sentiment using a pre-trained model (New)."""
    ...
    if os.path.exists(cache_file) and (time.time() - os.path.getmtime(cache_file)) < CONFIG['CACHE_EXPIRY_SECONDS']:
        ...
        # Placeholder: Replace with actual news fetching logic (e.g., API calls)
        news_text = "Sample news text for " + symbol  # Replace with real news data
...
def build_model(input_shape: Tuple[int, int]) -> tf.keras.Model:
    """Build a simplified neural network model with stronger regularization (New)."""
    K.clear_session()
    model = Sequential([
        Conv1D(32, kernel_size=3, activation='relu', input_shape=input_shape),  # Reduced from 64
        MaxPooling1D(pool_size=2),
        LSTM(64, return_sequences=True),  # Reduced from 128
        LSTM(64, return_sequences=False),  # Reduced from 128
        Dense(128, activation='relu', kernel_regularizer=l2(0.05)),  # Increased regularization
        Dropout(0.6),  # Increased dropout
        Dense(64, activation='relu', kernel_regularizer=l2(0.05)),  # Increased regularization
        Dropout(0.6),  # Increased dropout
...
def train_model(symbol: str, X: np.ndarray, y: np.ndarray, epochs: int, batch_size: int, progress_bar) -> Tuple[tf.keras.Model, StandardScaler]:
    ...
    for _ in range(epochs):
        model.fit(X_scaled_train, y_train, validation_data=(X_scaled_val, y_val), epochs=1, batch_size=batch_size, verbose=0)
...
def backtest(
    ...
    df: pd.DataFrame,
    initial_cash: float,
    stop_loss_atr_multiplier: float,  # Renamed for clarity
    take_profit_atr_multiplier: float,  # Renamed for clarity
    timesteps: int,
    buy_threshold: float,
    sell_threshold: float,
    min_holding_period_minutes: int,  # New parameter
    transaction_cost_per_trade: float  # New parameter
) -> Tuple[float, List[float], int, float]:
    """Run a backtest simulation with dynamic stop-losses, RSI filters, and transaction costs (Updated)."""
    ...
    position = 0
    entry_price = 0.0
    entry_time = None  # New: Track entry time for holding period
    returns = []
    trade_count = 0
    winning_trades = 0
    atr = df['ATR'].iloc[timesteps:].values
    prices = df['close'].iloc[timesteps:].values
    rsi = df['RSI'].iloc[timesteps:].values  # New: RSI for filtering
    ...
    for i in range(k_start, len(predictions)):
        ...
        # Check if we can buy: prediction > threshold and RSI < 30 (oversold)
        if pred > buy_threshold and position == 0 and qty > 0 and current_rsi < 30:
            position = qty
            entry_price = price
            entry_time = ts  # Record entry time
            cash -= qty * price + transaction_cost_per_trade  # Subtract transaction cost
            ...
            if (ts - entry_time).total_seconds() / 60 >= min_holding_period_minutes:
                # Sell if stop-loss or take-profit is hit, or if prediction < threshold and RSI > 70
                if price <= stop_loss or price >= take_profit or (pred < sell_threshold and current_rsi > 70):
                    cash += position * price - transaction_cost_per_trade  # Subtract transaction cost
...
def main(backtest_only: bool = False, force_train: bool = False) -> None:
    """Main function to execute the trading bot with portfolio-level risk management (Updated)."""
    ...
    if not backtest_only:
        portfolio_value = CONFIG['INITIAL_CASH']  # Track portfolio value for drawdown
        peak_value = portfolio_value  # Track peak portfolio value for drawdown
        ...
        while True:
            ...
            if clock.is_open:
                # Update portfolio value (cash + positions)
                ...
                for symbol in CONFIG['SYMBOLS']:
                    if symbol in models:
                        ...
                        if X.shape[0] > 0:
                            ...
                            current_rsi = df['RSI'].iloc[-1]  # New: RSI for filtering
                            try:
                                position = trading_client.get_open_position(symbol)
                                qty_owned = int(position.qty)
                                entry_time = pd.Timestamp(position.entry_time)  # Assuming entry_time is available
                            except:
                                qty_owned = 0
                                entry_time = None

                            # Buy condition with RSI filter
                            if prediction > CONFIG['PREDICTION_THRESHOLD_BUY'] and qty_owned == 0 and current_rsi < 30:
                            ...
                            # Sell condition with RSI filter and minimum holding period
                            elif qty_owned > 0:
                                ...
                                if time_held >= CONFIG['MIN_HOLDING_PERIOD_MINUTES']:
                                    if price <= stop_loss or price >= take_profit or (prediction < CONFIG['PREDICTION_THRESHOLD_SELL'] and current_rsi > 70):
