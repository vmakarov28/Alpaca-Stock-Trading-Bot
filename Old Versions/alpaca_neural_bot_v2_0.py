# alpaca_neural_bot_v2_0.py
# Version 2.0 - Enhanced to reduce overfitting with stronger regularization and data augmentation,
#               improved data fetching with customizable start dates and pagination, fixed live trading errors,
#               fixed data loading and training progress issues, fixed datetime comparison errors,
#               improved data fetching to load all available bars, and consolidated training progress bar.
#
# Possible Run Commands:
# 1. Backtest mode (using cached models):
#    python alpaca_neural_bot_v2_0.py --backtest
# 2. Backtest mode (forcing retraining):
#    python alpaca_neural_bot_v2_0.py --backtest --force-train
# 3. Live trading mode (using cached models):
#    python alpaca_neural_bot_v2_0.py
# 4. Live trading mode (forcing retraining):
#    python alpaca_neural_bot_v2_0.py --force-train
...
CONFIG = {
    ...
    'TRAIN_DATA_START_DATE': '2015-01-01',  # Customizable start date for training data
    'SIMULATION_DAYS': 365,
    'TRAIN_EPOCHS': 25,
    'BATCH_SIZE': 32,
    ...
    'LIVE_DATA_BARS': 200  # Number of recent bars to fetch in live trading
}
...
def fetch_data(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
    """Fetch historical bar data from Alpaca API with improved pagination to fetch all available bars."""
    try:
        client = StockHistoricalDataClient(CONFIG['ALPACA_API_KEY'], CONFIG['ALPACA_SECRET_KEY'])
        all_bars = []
        page_limit = 10000  # Alpaca's maximum page size
        start_dt = pd.Timestamp(start_date, tz='UTC')
        current_end = pd.Timestamp(end_date, tz='UTC')
        iteration = 0
        
        while True:
            iteration += 1
            logger.debug(f"Fetching data for {symbol}, iteration {iteration}, start: {start_dt}, end: {current_end}")
            ...
            if bars.empty:
                logger.debug(f"No more data available for {symbol} at iteration {iteration}")
                break
            ...
            # Update start_dt to the latest timestamp fetched plus 1 second
            latest_timestamp = df_bars['timestamp'].max()
            start_dt = latest_timestamp + pd.Timedelta(seconds=1)
            
            # If fewer than page_limit bars were returned, we've likely reached the end
            if len(bars) < page_limit:
                logger.debug(f"Fetched fewer than {page_limit} bars ({len(bars)}), ending fetch at iteration {iteration}")
                break
        ...
        logger.info(f"Total fetched {len(df)} bars for {symbol} with timeframe {CONFIG['TIMEFRAME']}, columns: {list(df.columns)}")
        return df
    except Exception as e:
...
def fetch_recent_data(symbol: str, num_bars: int) -> pd.DataFrame:
    """Fetch recent bars for live trading, ensuring enough data for sequences and indicators."""
    try:
        client = StockHistoricalDataClient(CONFIG['ALPACA_API_KEY'], CONFIG['ALPACA_SECRET_KEY'])
        end_date = datetime.now()
        # Fetch bars starting from 3 days ago to ensure enough data
...
def calculate_indicators(df: pd.DataFrame, sentiment: float) -> pd.DataFrame:
    ...
    try:
        ...
        if any(nan_counts.values()):
            ...
            # Drop rows with NaN values in indicators to ensure valid data
...
def load_model_and_scaler(symbol: str, expected_features: int, force_train: bool) -> Tuple[Optional[tf.keras.Model], Optional[StandardScaler]]:
    ...
    try:
        if force_train:
            logger.info(f"Force training enabled, skipping cache for {symbol}")
            return None, None
        ...
        if hasattr(scaler, 'n_features_in_') and scaler.n_features_in_ != expected_features:
            raise ValueError(
                ...
                f"but current feature set has {expected_features}. Run with --force-train to retrain."
            )
...
def main(backtest_only: bool = False, force_train: bool = False) -> None:
    ...
    try:
        ...
        dfs = {}
        training_metrics = {}
        stock_info = []  # To store info for printing after training
        
        # Only initialize progress bar if training will occur
        total_epochs = len(CONFIG['SYMBOLS']) * CONFIG['TRAIN_EPOCHS']
        progress_bar = None
        need_training = False
        
        # First pass: Check if training is needed
        ...
        if need_training:
            ...
            logger.info("All models loaded from cache, skipping training.")
        ...
        for symbol in CONFIG['SYMBOLS']:
            try:
                ...
                if not model_loaded:
                    ...
                    logger.info(f"Using cached model for {symbol}, training skipped.")
                
                models[symbol] = model
                scalers[symbol] = scaler
                
                # Collect stock info to print after training
                info = []
                ...
                info.append(f"  Calculated stop-loss multiplier: {CONFIG['STOP_LOSS_MULTIPLIER']:.2f}")
                try:
        ...
        # Print stock-specific info after training is complete
        for info in stock_info:
            for line in info:
                print(line)
            print()  # Add a newline between stocks
                        ...
                        if symbol in models:
                            try:
                                ...
                                if X.shape[0] > 0:
                                    ...
                                    last_sequence = X[-1]  # Shape: (TIMESTEPS, features)
                                    # Flatten the sequence for StandardScaler
                                    last_sequence_flat = last_sequence.reshape(1, -1)  # Shape: (1, TIMESTEPS * features)
                                    X_scaled = scalers[symbol].transform(last_sequence_flat)  # Shape: (1, TIMESTEPS * features)
                                    # Reshape back to 3D for model prediction
                                    X_scaled = X_scaled.reshape(1, CONFIG['TIMESTEPS'], len(features))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--force-train', action='store_true', help="Force retraining of models even if cached models exist")
