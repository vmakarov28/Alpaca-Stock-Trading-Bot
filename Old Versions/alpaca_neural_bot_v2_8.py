# +------------------------------------------------------------------------------+
# |                            Alpaca Neural Bot v2.8                            |
# +------------------------------------------------------------------------------+
# | Author: Vladimir Makarov                                                     |
# | Project Start Date: May 9, 2025                                              |
# | License: GNU Lesser General Public License v2.1                              |
# | Version: 2.8 (Un-Released)                                         |
# |                                                                              |
# | Description:                                                                 |
...
# |                                                                              |
# +------------------------------------------------------------------------------+
#Add emergancy stop feture to prevent overfitting during training when model stops improving
...
CONFIG = {
    'SYMBOLS': ['NVDA', 'TSLA', 'AAPL', 'MSFT', 'AMZN', 'SPY'], # List of stock symbols to trade
    'TIMEFRAME': TimeFrame(15, TimeFrameUnit.Minute), # Data interval for fetching market data (15-minute bars)
    'TRAIN_DATA_START_DATE': '2015-01-01', # Start date for historical data used in model training
    'SIMULATION_DAYS': 365, # Duration in days for backtesting the trading strategy
    'TRAIN_EPOCHS': 50, # Number of epochs to train the neural network model
    'BATCH_SIZE': 32, # Number of samples per batch during model training
    'INITIAL_CASH': 100000.00, # Starting capital for trading or backtesting
    'TIMESTEPS': 30, # Number of time steps in input sequences for the neural network
    'MIN_DATA_POINTS': 100, # Minimum number of data points required for valid training or trading
    'CACHE_DIR': './cache', # Directory to store cached data and models
    'CACHE_EXPIRY_SECONDS': 24 * 60 * 60, # Time in seconds before cached data expires (24 hours)
    'ALPACA_API_KEY': 'PK14JYOCMKTQHSJ0P1RZ', # Alpaca API key for accessing market data and trading
    'ALPACA_SECRET_KEY': 'tkQ4ELzvWm57pZMgAMntYIqKdzRogOPbG0mGJ4OF', # Alpaca secret key for authentication
    'EMAIL_SENDER': 'alpaca.ai.tradingbot@gmail.com', # Email address for sending trade notifications
    'EMAIL_PASSWORD': 'hjdf sstp pyne rotq', # Password for the sender email account
    'EMAIL_RECEIVER': ['aiplane.scientist@gmail.com', 'tchaikovskiy@hotmail.com'], # List of email addresses to receive notifications
    'SMTP_SERVER': 'smtp.gmail.com', # SMTP server for sending emails
    'SMTP_PORT': 587, # Port for the SMTP server
    'LOG_FILE': 'trades.log', # File to store trading logs
    'API_RETRY_ATTEMPTS': 3, # Number of retry attempts for failed API calls
    'API_RETRY_DELAY': 1000, # Delay in milliseconds between API retry attempts
    
    'EARLY_STOPPING_MONITOR': 'val_loss', # Metric to monitor for early stopping during training
    'LIVE_DATA_BARS': 200, # Number of recent bars to fetch for live trading
    'MIN_HOLDING_PERIOD_MINUTES': 60, # Minimum time to hold a position before selling (in minutes)
    'MAX_DRAWDOWN_LIMIT': 0.20, # Maximum allowable portfolio drawdown before pausing trading
    'TRANSACTION_COST_PER_TRADE': 1.0, # Fixed cost per trade for backtesting
'SENTIMENT_MODEL': 'distilbert-base-uncased-finetuned-sst-2-english', # Pre-trained model for news sentiment analysis
'TRAILING_STOP_PERCENTAGE': 0.02, # Percentage below the highest price to set a trailing stop
#-------- ⬇ CONFIGUE FOLLOWING VARIABLES TO CHANGE TRADING SYLES ⬇ -----------------------------
    'CONFIDENCE_THRESHOLD': 0.5, # Minimum model prediction confidence required to consider a trading decision
    'MAX_VOLATILITY': 4, # Maximum allowable price volatility to permit trading actions
    'ADX_TREND_THRESHOLD': 20, # Minimum ADX value indicating a strong trend to allow buy decisions
    'RISK_PERCENTAGE': 0.005, # Percentage of available cash to risk per trade for position sizing
    'STOP_LOSS_ATR_MULTIPLIER': 1.05, # Multiplier of ATR to set the stop-loss distance below entry price
    'TAKE_PROFIT_ATR_MULTIPLIER': 1.25, # Multiplier of ATR to set the take-profit target above entry price
    'PREDICTION_THRESHOLD_BUY': 0.55, # Minimum model prediction confidence for a buy decision
    'PREDICTION_THRESHOLD_SELL': 0.45, # Maximum model prediction confidence for a sell decision
    'RSI_BUY_THRESHOLD': 50,
    'EARLY_STOPPING_PATIENCE': 5, #If training does not improve after this many epochs; training will stop
    'EARLY_STOPPING_MIN_DELTA': 0.0, #Minimum improvment threshold. If 0 then any improvment is taken
}
...
def train_model(symbol: str, X: np.ndarray, y: np.ndarray, epochs: int, batch_size: int, progress_bar) -> Tuple[tf.keras.Model, StandardScaler]:
    """Train the model with early stopping and emergency stop logging."""
    ...
    if len(history.history[CONFIG['EARLY_STOPPING_MONITOR']]) < epochs:
        logger.info(f"Emergency stop for {symbol}: Training halted after {len(history.history[CONFIG['EARLY_STOPPING_MONITOR']])} epochs due to no improvement in {CONFIG['EARLY_STOPPING_MONITOR']}.")
...
def backtest(
    ...
    for i in range(k_start, len(predictions)):
        ...
        # Adjust qty if cost exceeds cash (including transaction cost)
        if qty > 0:
            cost = qty * price + transaction_cost_per_trade
            if cost > cash:
                # Adjust qty to use all available cash, accounting for transaction cost
                qty = int((cash - transaction_cost_per_trade) / price) if (cash - transaction_cost_per_trade) >= price else 0

        if pred > buy_threshold and position == 0 and qty > 0 and current_rsi < CONFIG['RSI_BUY_THRESHOLD'] and current_adx > adx_trend_threshold:
            position = qty
            entry_price = price
            max_price = price
            entry_time = ts
...
@tf.function(input_signature=[
    tf.TensorSpec(shape=None, dtype=tf.float32),  # Model (opaque object, use None for shape)
    tf.TensorSpec(shape=[1, 30, 25], dtype=tf.float32)  # X_scaled: (1, TIMESTEPS, features)
])
def make_prediction(model, X_scaled):
    """Make a prediction using the model with fixed input signature to minimize retracing."""
    return model(X_scaled, training=False)[0][0]
                ...
                for symbol in CONFIG['SYMBOLS']:
                    if symbol in models:
                        ...
                        if X.shape[0] > 0:
                            ...
                            prediction = make_prediction(models[symbol], tf.convert_to_tensor(X_scaled, dtype=tf.float32))
                            ...
                            if position:
                                qty_owned = int(position.qty)
                                entry_time = pd.Timestamp(position.entry_time) if hasattr(position, 'entry_time') else now
                                entry_price = float(position.avg_entry_price)
                                time_held = (now - entry_time).total_seconds() / 60

                            decision = "Hold"
                            if current_volatility <= CONFIG['MAX_VOLATILITY'] and current_adx >= CONFIG['ADX_TREND_THRESHOLD'] and prediction >= CONFIG['CONFIDENCE_THRESHOLD']:
                                if prediction > CONFIG['PREDICTION_THRESHOLD_BUY'] and qty_owned == 0 and current_rsi < 30 and current_adx > CONFIG['ADX_TREND_THRESHOLD']:
                                    decision = "Buy"
                            if decision == "Buy" or (decision == "Hold" and qty_owned == 0):
                                cash = float(account.cash)
                                max_qty = int(float(account.buying_power) / price)  # Maximum affordable shares
                                risk_amount = cash * CONFIG['RISK_PERCENTAGE']  # Amount to risk
                                qty = int(min(risk_amount / (atr_val * CONFIG['STOP_LOSS_ATR_MULTIPLIER']), max_qty * (100 / price)))  # Scale by inverse price
